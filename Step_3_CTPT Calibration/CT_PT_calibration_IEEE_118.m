...
%% Conduct CT?PT calibration based on two bus equipped with accurate PMUs
...
% VI_origin(:,2) = quant(real(VI_origin_true(:,2)),0.65) + 1i*quant(imag(VI_origin_true(:,2)),0.65);
...
line_estimation_results(accurate_line_index,2:3) = [accurate_bus, original_KV1];
...
%                 (current_injection_line_number_set(:,1)~=current_accurate_line_number,:);
            current_inaccurate_line_numbers = current_KV_injection_set(:,1);
...
%                     line_estimation_results(results_set_index,4) = current_KI_injection_set(k,2);
            ...
            Z = line_estimation_results(results_set_index,8);  
            y = line_estimation_results(results_set_index,9); 
            
            [KI1,KV2,KI2]=single_line_CT_PT_calibration...
                (current_line_number,current_bus_number,KV1,line_bus_info_345KV(results_set_index,2:3),Z,y);
            
            line_estimation_results(results_set_index,4) = KI1;
...
table(line_number, KV1_error,KI1_error,KV2_error,KI2_error)
